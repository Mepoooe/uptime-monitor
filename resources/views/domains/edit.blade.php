...
@section('content')
<div class="max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Domain</h1>
        ...
        <form method="POST" action="{{ route('domains.update', $domain) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="check_interval"
                        ...
                        @foreach($intervals as $interval)
                            <option value="{{ $interval }}" {{ old('check_interval', $domain->check_interval) == $interval ? 'selected' : '' }}>
                                every {{ $interval }} min
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="check_timeout" value="{{ old('check_timeout', $domain->check_timeout) }}"
                           min="1" max="60"
                           class="w-full rounded-md border-gray-300 border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>
